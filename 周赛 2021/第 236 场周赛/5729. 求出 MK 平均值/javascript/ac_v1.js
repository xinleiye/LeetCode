...
MKAverage.prototype.addElement = function(num) {
    ...
    if (total >= this.m) {
        let index = binarySearch(this.dataBuff, this.dataM, this.dataBuff[total - this.m]);
        ...
        } else if (index === this.m - 1 || num < this.dataBuff[this.dataM[index - 1]]) {
...
function move2End (numArr, indexArr, index) {
    ...
    while (index < indexArr.length - 1 && numArr[indexArr[index]] > numArr[indexArr[index + 1]]) {
        tmp = indexArr[index];
        indexArr[index] = indexArr[index + 1];
        indexArr[index + 1] = tmp;
        index++;
